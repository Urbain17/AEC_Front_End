...
function FormEditerAutoHooks(props) {
    const [donneesRecues, setDonneesRecues] = useState({ name: '', picture: "", abilities: ["", ""] });
    const [pokemonID, setAutoID] = useState(props.location.search.substring(4, props.location.search.length));
    ...
    async function editAuto(nom, photo, attaque1, attaque2) {
        try {
            const response = await fetch(API + pokemonID, {
                method: 'PUT',
    ...
    function handleEdit(event) {
        ...
        editAuto(nom, photo, attaque1, attaque2);
    }
    ...
    return ( <
            ...
            <
            Form.Label > Nom de l 'auto< /Form.Label> <
            Form.Control type = "text"
            defaultValue = { donneesRecues.name }
            /> {/ * / Faire le test avec value*/
        } <
        /Form.Group> <
    Form.Group controlId = "photoPokemon" >
        <
        Form.Label > URL d 'une photo auto< /Form.Label> <
    Form.Control type = "text"
    placeholder = "Entrer une URL valide"
    onBlur = { handlePhoto }
    defaultValue = { donneesRecues.picture }
    /> < /
    Form.Group > {
            donneesRecues.picture !== "" && < Image src = { donneesRecues.picture }
            rounded width = "125" / >
        } <
    ...
    Form.Control type = "text"
    placeholder = "Entrer le nom de l'attaque 1"
    defaultValue = { donneesRecues.abilities[0].name }
    /> < /
    Form.Group > <
    ...
    Form.Control type = "text"
    placeholder = "Entrer le nom de l'attaque 2"
    defaultValue = { donneesRecues.abilities[1].name }
    /> < /
    Form.Group >
    ...
    Form > <
        /Col>     < /
    Row > <
        p className = "btn btn-danger mt-5"
    onClick = { removeAuto } > Supprimer auto < /p> < /
    Container > <
        />
);
}

export default FormEditerAutoHooks;