...
/*$(document).ready(function(){
    $( "#form" ).submit(function( event ) {
        if($("#alphabetPhoto").val().length < 3 || $("#alphabetPhoto").val().length > 12 ){
            ...
            for(var i=0;i<text.length;i++){
                var lettreId = Math.floor(Math.random() * 5)+1;
                var colone = "col-"+colsize;
                var image = text[i]+""+lettreId
                var lettre= '</div><div class="__col"><img src="Letters/__path/__image.jpg" class="img-responsive letters" id="Letter6"></div>';


                lettre = lettre.replace('__col',colone).replace(/__image/g, image.toUpperCase()).replace(/__path/g,text[i]);
